...
//						 		    2. videoId
YouTubeVideoFile.createMediaFiles = function(doc, tab) {
	...
	YouTubeVideoFile.createMediaFilesByTypeOfRequest(isAjaxVideo, doc, tab, function(doc, tab, responseText) {
		...
		for (var i = 0; i < youTubeVideoParams.length; i++) {
		    ...
		    mediaFile.addToMediaList(mediaFile, tab);
		}
			
		try {
			MediaFile.log("yo yo momo");
		    if (tab.videoFilesList[0].s) {
	    		i = i || 0;
		    	// var sigDec = new SigDec();
		    	SigDec.init(youTubeVideoParams, responseText, doc);
		    	SigDec.decipher(tab.videoFilesList, function() {
		    		MediaFile.log("done");
		    		MediaFile.log(tab.videoFilesList);
		    	});		    
		    }
		} catch(ex) { MediaFile.log(ex); }			
	});		
...
YouTubeVideoFile.createMediaFilesByTypeOfRequest = function(isAjaxVideo, doc, tab, callback) {
	if (!isAjaxVideo) {
	...
	YouTubeVideoFile.getAjaxNavigatedVideosFile(videoId, doc, tab, callback);
	// log(videoId);
};

YouTubeVideoFile.getAjaxNavigatedVideosFile = function(videoId, doc, tab, callback) {
    ...
    ajaxRequest.channel.loadFlags |= Components.interfaces.nsIRequest.LOAD_ANONYMOUS | Components.interfaces.nsIRequest.LOAD_BYPASS_CACHE | Components.interfaces.nsIRequest.INHIBIT_PERSISTENT_CACHING;
    
    ajaxRequest.onreadystatechange = function() {            
        try {
            if (this.readyState == 4 && this.status == 200) {
            	tab.videoFilesList = new Array();
                callback(doc, tab, this.responseText);                      
            }
        } 
        catch(ex) { MediaFile.log(ex); }
    }
